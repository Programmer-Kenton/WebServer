...
int initSocket(short port){
    ...
    //如果服务器因为某些异常原因终止,需要重启,可能因为端口未释放,而导致重启失败
    //所以需要对端口进行设置,允许端口被多个套接字绑定,端口复用
    printf("%d.%ld > 设置套接字\n",getpid(),syscall(SYS_gettid));
    int on = 1;
...
char* recvRequest(int conn){
    ...
    for(;;){
        ...
        req = static_cast<char *>(realloc(req, len + size + 1));//扩大存储区
        memcpy(req + len,buf,size + 1);//拷贝此次接受数据到存储区中
        len = len + size;//总长度累加
        //如果请求中有两个\r\n,代表接受完成
        if(strstr(req,"\r\n\r\n")){
            break;
